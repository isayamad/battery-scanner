...
class BatteryApp(App):
    def build(self):
        ...
        btn_scan = Button(text="📷 Enter Battery ID", size_hint_y=0.15, font_size=18)
        btn_scan.bind(on_press=self.manual_entry)
    ...
    def get_count(self):
        if not XL_OK or not os.path.exists(self.excel_path):
            return "Total: 0"
